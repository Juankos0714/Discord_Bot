...
// Endpoint para enviar directamente a Discord
app.post('/api/discord', async (req, res) => {
    ...
    if (!message || !message.trim()) {
        return res.status(400).json({ 
            success: false, 
            error: 'Por favor, proporciona un mensaje válido.' 
        });
    }
    ...
    if (!client || !process.env.DISCORD_CHANNEL_ID) {
        return res.status(500).json({ 
            success: false, 
            error: 'Discord no está configurado correctamente.' 
        });
    }
    ...
    try {
        ...
        if (!channel) {
            return res.status(500).json({ 
                success: false, 
                error: 'Canal de Discord no encontrado.' 
            });
        }
        ...
        console.log('✅ Mensaje directo enviado a Discord exitosamente');
        
    } catch (error) {
        console.error('❌ Error enviando mensaje directo a Discord:', error);
        res.status(500).json({ 
            success: false, 
            error: 'Error al enviar mensaje a Discord: ' + error.message 
        });
    }
});
...
const server = app.listen(PORT, '0.0.0.0', () => {
    ...
    if (err.code === 'EADDRINUSE') {
        console.error(`❌ Puerto ${PORT} ya está en uso. Intentando con otro puerto...`);
        
        // Intentar con un puerto aleatorio
        const alternativePort = PORT + Math.floor(Math.random() * 1000) + 1;
        console.log(`🔄 Intentando con puerto ${alternativePort}`);
        
        const alternativeServer = app.listen(alternativePort, '0.0.0.0', () => {
            console.log(`🚀 Servidor ejecutándose en puerto ${alternativePort}`);
            console.log(`📁 Sirviendo archivos desde: ${__dirname}`);
            if (process.env.DISCORD_TOKEN) {
                console.log('✅ Discord configurado correctamente');
            }
